...
class openSetClassifier(nn.Module):
	def __init__(self, num_classes = 20, im_size = 64, clip = "clip", device = "cpu", init_weights = False, **kwargs):
		...
		if clip == "clip":
			...
			import open_clip
			model, _, _ = open_clip.create_model_and_transforms('hf-hub:imageomics/bioclip', device=device, jit=False)
	...
	def forward(self, x, skip_distance = False):
		batch_size = len(x)
		x = self.encoder.encode_image(x).float()
